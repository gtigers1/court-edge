...
function NBAPage(){
  ...
  const fetchOdds=async()=>{setOddsLoading(true);setOddsError("");setSharpAlert(null);try{const r=await fetch("/api/odds",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sport:"nba",homeTeam,awayTeam})});const d=await r.json();if(!r.ok)throw new Error(d.error||"Error "+r.status);if(d.homeML)setHomeOdds(d.homeML);if(d.awayML)setAwayOdds(d.awayML);if(d.homeSpread)setHomeSpread(d.homeSpread);if(d.total)setPostedTotal(d.total);if(d.sharpIndicator)setSharpAlert(d.sharpIndicator);}catch(e){setOddsError(e.message);}setOddsLoading(false);};
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {bothLoaded&&<div className="fade-in" style={{background:C.card,border:"1px solid "+C.border,borderRadius:12,overflow:"hidden"}}>
      ...
      <div style={{padding:14}}>
        ...
        <button className="hov-btn" onClick={runModels} disabled={analyzing} style={{width:"100%",padding:"13px 0",background:analyzing?C.dim:"linear-gradient(90deg,"+C.copper+","+C.copperL+")",border:"none",borderRadius:9,cursor:analyzing?"not-allowed":"pointer",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:17,letterSpacing:2,color:analyzing?C.muted:C.black,textTransform:"uppercase"}}>{analyzing?<span className="pulse">Running Models...</span>:"Run Analysis"}</button>
      </div>
    </div>}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {bothLoaded&&<div className="fade-in" style={{background:C.card,border:"1px solid "+C.border,borderRadius:12,overflow:"hidden"}}>
      ...
      <div style={{padding:14}}>
        ...
        <button className="hov-btn" onClick={runModels} disabled={analyzing} style={{width:"100%",padding:"13px 0",background:analyzing?C.dim:"linear-gradient(90deg,"+C.iceD+","+C.ice+")",border:"none",borderRadius:9,cursor:analyzing?"not-allowed":"pointer",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:17,letterSpacing:2,color:analyzing?C.muted:C.black,textTransform:"uppercase"}}>{analyzing?<span className="pulse">Running Models...</span>:"Run Analysis"}</button>
      </div>
    </div>}
