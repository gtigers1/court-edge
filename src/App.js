...
const STYLES=`
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .hov-card:hover{transform:translateY(-2px);border-color:#B87333!important;}
...
function ModelCard({icon,name,desc,awayTeam,homeTeam,awayAbbr,homeAbbr,awayProb,detail,accent}){
  ...
  return <div className="hov-card" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:14,display:"flex",flexDirection:"column",gap:10}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:6,flex:1}}><Badge abbr={awayAbbr} size={28} accent={ac}/><div><div style={{fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:20,color:awayFav?ac:C.white,lineHeight:1}}>{(awayProb*100).toFixed(0)}%</div><div style={{fontSize:9,color:C.muted}}>{awayTeam.split(" ").at(-1)}</div></div></div>
      <div style={{width:1,height:32,background:C.border}}/>
      <div style={{display:"flex",alignItems:"center",gap:6,flex:1,justifyContent:"flex-end"}}><div style={{textAlign:"right"}}><div style={{fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:20,color:!awayFav?ac:C.white,lineHeight:1}}>{(hp*100).toFixed(0)}%</div><div style={{fontSize:9,color:C.muted}}>{homeTeam.split(" ").at(-1)}</div></div><Badge abbr={homeAbbr} size={28} accent={ac}/></div>
    </div>
...
function NBAPage(){
  const [homeTeam,setHomeTeam]=useState("");const [awayTeam,setAwayTeam]=useState("");const [homeOdds,setHomeOdds]=useState("");const [awayOdds,setAwayOdds]=useState("");const [homeData,setHomeData]=useState(null);const [awayData,setAwayData]=useState(null);const [loading,setLoading]=useState(false);const [error,setError]=useState("");const [results,setResults]=useState(null);const [tab,setTab]=useState("results");const [dataLoaded,setDataLoaded]=useState(false);
  ...
  const runModels=(h,a)=>{const elo=nbaMdlElo(h,a),rap=nbaMdlRaptor(h,a),ff=nbaMdlFF(h,a),ml=nbaMdlML(h,a),mc=nbaMdlMC(h,a);setResults({elo,rap,ff,ml,mc,cons:nbaConsensus([elo.homeProb,rap.homeProb,ff.homeProb,ml.homeProb,mc.homeProb])});setTab("results");};
  const handleFetch=async()=>{if(!homeTeam||!awayTeam){setError("Select both teams.");return;}if(homeTeam===awayTeam){setError("Select two different teams.");return;}setError("");setLoading(true);setResults(null);setDataLoaded(false);try{const r=await fetch("/api/matchup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({homeTeam,awayTeam})});const d=await r.json();if(!r.ok)throw new Error(d.error||`Error ${r.status}`);if(!d.home||!d.away)throw new Error("Invalid response");setHomeData(d.home);setAwayData(d.away);setDataLoaded(true);runModels(d.home,d.away);}catch(e){setError(`Failed: ${e.message}`);}setLoading(false);};
  ...
  return <div>
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",marginBottom:14}}>
      ...
      <div style={{padding:16}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:12,alignItems:"center",marginBottom:14}}>
          <div><div style={{fontSize:10,color:C.teal,fontFamily:"'Barlow Condensed'",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>Away Team</div><select style={{...inp,cursor:"pointer"}} value={awayTeam} onChange={e=>{setAwayTeam(e.target.value);setAwayData(null);setResults(null);setDataLoaded(false);}}><option value="">Select team...</option>{NBA_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",paddingTop:20}}><div style={{width:36,height:36,borderRadius:"50%",background:C.dark,border:`2px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:13,color:C.muted}}>@</div></div>
          <div><div style={{fontSize:10,color:C.copper,fontFamily:"'Barlow Condensed'",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>Home Team</div><select style={{...inp,cursor:"pointer"}} value={homeTeam} onChange={e=>{setHomeTeam(e.target.value);setHomeData(null);setResults(null);setDataLoaded(false);}}><option value="">Select team...</option>{NBA_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
        </div>
        ...
        {error&&<div style={{padding:"10px 14px",background:"#2a0f0f",border:"1px solid #5a2020",borderRadius:8,marginBottom:12,fontSize:12,color:"#f87171"}}>Warning: {error}</div>}
        <button className="hov-btn" onClick={handleFetch} disabled={loading} style={{width:"100%",padding:"13px 0",background:loading?C.dim:`linear-gradient(90deg,${C.copper},${C.copperL})`,border:"none",borderRadius:9,cursor:loading?"not-allowed":"pointer",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:17,letterSpacing:2,color:loading?C.muted:C.black,textTransform:"uppercase"}}>{loading?<span className="pulse">Fetching Live 2025-26 Data...</span>:"Fetch Live Data & Analyze"}</button>
    ...
    {dataLoaded&&homeData&&awayData&&<div className="fade-in" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",marginBottom:14}}>
      <SectionHeader label="Injury Report" accent={C.teal} right={<div style={{display:"flex",gap:6}}>{outCount(awayData)>0?<Pill label={`${NBA_ABBR[awayTeam]} ${outCount(awayData)} OUT`} color="#E05252"/>:<Pill label={`${NBA_ABBR[awayTeam]} FULL`} color={C.teal}/>}{outCount(homeData)>0?<Pill label={`${NBA_ABBR[homeTeam]} ${outCount(homeData)} OUT`} color="#E05252"/>:<Pill label={`${NBA_ABBR[homeTeam]} FULL`} color={C.teal}/>}</div>}/>
      <div style={{padding:14}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><RosterPanel teamName={awayTeam} abbr={NBA_ABBR[awayTeam]} teamData={awayData} onCycle={n=>cyclePlayer("away",n)} sport="nba" accent={C.teal}/><RosterPanel teamName={homeTeam} abbr={NBA_ABBR[homeTeam]} teamData={homeData} onCycle={n=>cyclePlayer("home",n)} sport="nba" accent={C.teal}/></div><button className="hov-btn" onClick={()=>runModels(homeData,awayData)} style={{marginTop:12,width:"100%",padding:"11px 0",background:`linear-gradient(90deg,${C.tealD},${C.teal})`,border:"none",borderRadius:8,cursor:"pointer",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:15,letterSpacing:2,color:C.black,textTransform:"uppercase"}}>Recalculate With Updated Injuries</button></div>
    </div>}
    {results&&<NBAResults results={results} awayTeam={awayTeam} homeTeam={homeTeam} awayAbbr={NBA_ABBR[awayTeam]} homeAbbr={NBA_ABBR[homeTeam]} awayOdds={awayOdds} homeOdds={homeOdds} tab={tab} setTab={setTab}/>}
  </div>;
}

function NBAResults({results,awayTeam,homeTeam,awayAbbr,homeAbbr,awayOdds,homeOdds,tab,setTab}){
    ...
    {tab==="results"&&<>
      <div style={{background:C.card,border:`1.5px solid ${C.copper}44`,borderRadius:12,padding:20,marginBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:16,alignItems:"center",marginBottom:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}><div style={{fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:14,color:C.muted,letterSpacing:2}}>VS</div>{(aE||hE)&&<div style={{textAlign:"center",padding:"8px 12px",background:C.black,border:`1px solid ${C.border}`,borderRadius:8}}><div style={{fontFamily:"'Barlow Condensed'",fontSize:11,fontWeight:800,color:C.muted,letterSpacing:1}}>{sig(parseFloat(aw>cH?aE:hE)).l}</div></div>}</div>
...
function nhlMdlGoalie(h,a){const gPen=g=>(g&&g.status&&g.status!=="PLAYING")?((g.status==="OUT"?1:g.status==="DOUBTFUL"?.7:.35)*(.920-(g.save_pct||.905))):0;const hSv=(h.goalie?.save_pct||.905)-gPen(h.goalie),aSv=(a.goalie?.save_pct||.905)-gPen(a.goalie);const hGA=(h.shots_against_pg||30)*(1-hSv),aGA=(a.shots_against_pg||30)*(1-aSv);return{homeProb:Math.min(.97,Math.max(.03,logistic((aGA-hGA)*1.2+.05))),hSv:hSv.toFixed(3),aSv:aSv.toFixed(3),hGA:hGA.toFixed(2),aGA:aGA.toFixed(2)};}
function nhlMdlSpecialTeams(h,a){const hST=(h.pp_pct||20)/100*.6+(h.pk_pct||80)/100*.4,aST=(a.pp_pct||20)/100*.6+(a.pk_pct||80)/100*.4;const injPen=r=>(r||[]).reduce((s,p)=>p.status==="PLAYING"?s:s+(p.status==="OUT"?1:p.status==="DOUBTFUL"?.6:.3)*(p.role==="STAR"?.06:p.role==="KEY"?.03:.01),0);return{homeProb:Math.min(.97,Math.max(.03,logistic((hST-aST)*3+(injPen(a.roster)-injPen(h.roster))+.05))),hST:(hST*100).toFixed(1),aST:(aST*100).toFixed(1)};}
...
function nhlMdlMC(h,a,N=8000){const gAdj=d=>d.goalie?.status==="OUT"?.4:d.goalie?.status==="DOUBTFUL"?.2:d.goalie?.status==="QUESTIONABLE"?.1:0;const iAdj=r=>(r||[]).reduce((s,p)=>p.status==="PLAYING"?s:s+(p.status==="OUT"?1:p.status==="DOUBTFUL"?.6:.3)*(p.role==="STAR"?.08:p.role==="KEY"?.04:.01),0);const hExp=h.gf_pg-gAdj(h)-iAdj(h.roster)+.1,aExp=a.gf_pg-gAdj(a)-iAdj(a.roster);let hw=0;for(let i=0;i<N;i++){const hG=Math.max(0,hExp+Math.sqrt(Math.max(hExp,.5))*(Math.sqrt(-2*Math.log(Math.random()))*Math.cos(2*Math.PI*Math.random())));const aG=Math.max(0,aExp+Math.sqrt(Math.max(aExp,.5))*(Math.sqrt(-2*Math.log(Math.random()))*Math.cos(2*Math.PI*Math.random())));hw+=hG>aG?1:hG===aG?.5:0;}return{homeProb:Math.min(.97,Math.max(.03,hw/N)),hExp:hExp.toFixed(2),aExp:aExp.toFixed(2)};}
...
function NHLPage(){
  ...
  const runModels=(h,a)=>{const elo=nhlMdlElo(h,a),gl=nhlMdlGoalie(h,a),st=nhlMdlSpecialTeams(h,a),cs=nhlMdlCorsi(h,a),mc=nhlMdlMC(h,a);setResults({elo,gl,st,cs,mc,cons:nhlConsensus([elo.homeProb,gl.homeProb,st.homeProb,cs.homeProb,mc.homeProb])});setTab("results");};
  const handleFetch=async()=>{if(!homeTeam||!awayTeam){setError("Select both teams.");return;}if(homeTeam===awayTeam){setError("Select two different teams.");return;}setError("");setLoading(true);setResults(null);setDataLoaded(false);try{const r=await fetch("/api/nhl-matchup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({homeTeam,awayTeam})});const d=await r.json();if(!r.ok)throw new Error(d.error||`Error ${r.status}`);if(!d.home||!d.away)throw new Error("Invalid response");setHomeData(d.home);setAwayData(d.away);setDataLoaded(true);runModels(d.home,d.away);}catch(e){setError(`Failed: ${e.message}`);}setLoading(false);};
  const inp={width:"100%",padding:"10px 12px",background:C.black,border:`1.5px solid ${C.border}`,borderRadius:8,color:C.white,fontSize:13,outline:"none",fontFamily:"'Barlow',sans-serif"};
  const outCount=d=>d?.roster?.filter(p=>p.status!=="PLAYING").length||0;
  ...
  return <div>
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",marginBottom:14}}>
      ...
      <div style={{padding:16}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:12,alignItems:"center",marginBottom:14}}>
          <div><div style={{fontSize:10,color:C.ice,fontFamily:"'Barlow Condensed'",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>Away Team</div><select style={{...inp,cursor:"pointer"}} value={awayTeam} onChange={e=>{setAwayTeam(e.target.value);setAwayData(null);setResults(null);setDataLoaded(false);}}><option value="">Select team...</option>{NHL_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",paddingTop:20}}><div style={{width:36,height:36,borderRadius:"50%",background:C.dark,border:`2px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:13,color:C.muted}}>@</div></div>
          <div><div style={{fontSize:10,color:C.copper,fontFamily:"'Barlow Condensed'",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>Home Team</div><select style={{...inp,cursor:"pointer"}} value={homeTeam} onChange={e=>{setHomeTeam(e.target.value);setHomeData(null);setResults(null);setDataLoaded(false);}}><option value="">Select team...</option>{NHL_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 36px 1fr",gap:12,marginBottom:14}}>
          <div><div style={{fontSize:10,color:C.muted,letterSpacing:1,marginBottom:5,textTransform:"uppercase"}}>Away Moneyline</div><input style={inp} placeholder="+130" value={awayOdds} onChange={e=>setAwayOdds(e.target.value)}/></div>
          <div/>
        ...
        {error&&<div style={{padding:"10px 14px",background:"#2a0f0f",border:"1px solid #5a2020",borderRadius:8,marginBottom:12,fontSize:12,color:"#f87171"}}>Warning: {error}</div>}
        <button className="hov-btn" onClick={handleFetch} disabled={loading} style={{width:"100%",padding:"13px 0",background:loading?C.dim:`linear-gradient(90deg,${C.iceD},${C.ice})`,border:"none",borderRadius:9,cursor:loading?"not-allowed":"pointer",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:17,letterSpacing:2,color:loading?C.muted:C.black,textTransform:"uppercase"}}>{loading?<span className="pulse">Fetching Live 2025-26 Data...</span>:"Fetch Live Data & Analyze"}</button>
    ...
    {dataLoaded&&homeData&&awayData&&<div className="fade-in" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",marginBottom:14}}>
      <SectionHeader label="Lineup & Injury Report" accent={C.ice} right={<div style={{display:"flex",gap:6}}>{(outCount(awayData)>0||goalieOut(awayData))?<Pill label={`${NHL_ABBR[awayTeam]} ${outCount(awayData)+(goalieOut(awayData)?1:0)} OUT`} color="#E05252"/>:<Pill label={`${NHL_ABBR[awayTeam]} FULL`} color={C.ice}/>}{(outCount(homeData)>0||goalieOut(homeData))?<Pill label={`${NHL_ABBR[homeTeam]} ${outCount(homeData)+(goalieOut(homeData)?1:0)} OUT`} color="#E05252"/>:<Pill label={`${NHL_ABBR[homeTeam]} FULL`} color={C.ice}/>}</div>}/>
      <div style={{padding:14}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><RosterPanel teamName={awayTeam} abbr={NHL_ABBR[awayTeam]} teamData={awayData} onCycle={n=>cyclePlayer("away",n)} sport="nhl" accent={C.ice}/><RosterPanel teamName={homeTeam} abbr={NHL_ABBR[homeTeam]} teamData={homeData} onCycle={n=>cyclePlayer("home",n)} sport="nhl" accent={C.ice}/></div><button className="hov-btn" onClick={()=>runModels(homeData,awayData)} style={{marginTop:12,width:"100%",padding:"11px 0",background:`linear-gradient(90deg,${C.iceD},${C.ice})`,border:"none",borderRadius:8,cursor:"pointer",fontFamily:"'Barlow Condensed'",fontWeight:900,fontSize:15,letterSpacing:2,color:C.black,textTransform:"uppercase"}}>Recalculate With Updated Lineup</button></div>
    </div>}
